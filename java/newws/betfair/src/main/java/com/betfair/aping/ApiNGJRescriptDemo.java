...
public class ApiNGJRescriptDemo {
	...
	private ApiNgOperations rescriptOperations = ApiNgRescriptOperations
			.getInstance();
	...
	public void start(String appKey, String ssoid) {
		...
		try {
			...
			List<EventTypeResult> r = rescriptOperations.listEventTypes(
					marketFilter, applicationKey, sessionToken);
			System.out
					.println("2. Extract Event Type Id for Horse Racing...\n");
			for (EventTypeResult eventTypeResult : r) {
				if (eventTypeResult.getEventType().getName()
						.equals("Horse Racing")) {
					System.out
							.println("3. EventTypeId for \"Horse Racing\" is: "
									+ eventTypeResult.getEventType().getId()
									+ "\n");
			 ...
			 */
			System.out
					.println("4.(listMarketCataloque) Get next horse racing market in the UK...\n");
			...
			// In this case we don't need these objects so they are declared
			// null
			...
			List<MarketBook> marketBookReturn = rescriptOperations
			...
			if (marketBookReturn.size() != 0) {
				...
				System.out
						.println("7. Place a bet below minimum stake to prevent the bet actually "
								+ "being placed for marketId: "
								+ marketIdChosen
								+ " with selectionId: "
								+ selectionId + "...\n\n");
				...
				// API-NG will return an error with the default size=0.01. This
				// is an expected behaviour.
				// You can adjust the size and price value in the
				// "apingdemo.properties" file
				...
				PlaceExecutionReport placeBetResult = rescriptOperations
						.placeOrders(marketIdChosen, instructions, customerRef,
								applicationKey, sessionToken);
				...
				if (placeBetResult.getStatus() == ExecutionReportStatus.SUCCESS) {
					...
					System.out.println("The error is: "
							...
							+ placeBetResult.getErrorCode().getMessage());
				}
			} else {
