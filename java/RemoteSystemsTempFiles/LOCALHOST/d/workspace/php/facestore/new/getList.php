...
while ($row = mysql_fetch_assoc($res)) {
		...
		$table.="<td bgcolor='#ffffff' align=left><span class='imgname'>$row[name]<br><img width='134' style='cursor:pointer;' onclick='popBigImg($row[id],$rc)' height='154' src='$row[img]'".
	        "/><br>$row[price] TL</span><br>";

	if($page->orderButton){
//		$table.= " <input type='button' onclick='poppaypal($row[id],$rc)' value='Satın Al' /><br>".
		$table.= " <img src=img/satinal.gif align=absmiddle onclick='poppaypal($row[id],$rc)' border=0 alt='Satın Al'><br>".
