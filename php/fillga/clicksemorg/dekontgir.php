...
if(isset($_POST['submit'])){
	...
	$user=$dapi->select("select bakiye from users where customerId=$_POST[account]");
	$user=$user[0];
	$curbak=$user['bakiye'];
	$dapi->insert("insert into muhasebe set customerId=$_POST[account],aciklama='$_POST[aciklama]', type='dekont',borc=$_POST[bakiye],curbak=$curbak, tarih='".date('Y-m-d H:i:s')."'");
