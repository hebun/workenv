...
$con;
if($isLocal===0){
	$con=mysqli_connect("localhost","clicks_banner","Shedower83!");
...
class Dapi{
	...
	public function Dapi()
	{
		...
		if($isLocal==1){
			...
			$this->mysqli = new mysqli("localhost","clicks_banner","Shedower83!","adwords");
	...
	{
		$this->fillgaStmt->bind_param('issssssdiisddds',
			$arr->tarih,
			$arr->campaign,
			$arr["account"], 
			$arr["keywordState"],
