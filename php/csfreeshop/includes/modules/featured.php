...
// *************************************
			if	(IMG_HOVER_EFFECT == 'style-1')	{
				$img_effect = 'first';
				$img_effect_width = PRODS_BLOCK_FEATURED_MODUL_WIDTH;	
			}
...
// *************************************
    ...
    $product = tep_db_fetch_array($product_query);
		...
		$p_desc =  mb_substr(strip_tags($product['products_description']), 0, MAX_DESCR_FEATURED_MODUL, 'UTF-8').'...';
	 ...
	 $p_price = $lc_text = '<span class="productSpecialPrice un">' . $currencies->display_price($featured_products['specials_new_products_price'], tep_get_tax_rate($featured_products['products_tax_class_id'])) . '</span>
...
// *************************************	  
    $featured_prods_content .= '<li style="width:'.PRODS_BLOCK_FEATURED_MODUL_WIDTH.'px;" class="wrapper_prods equal-height_featured_block hover">'.
				  '<div class="border_prods">'. "\n".
					''. "\n".
					'<div class="pic_padd wrapper_pic_div" style="width:'.(FEATURED_MODUL_IMAGE_WIDTH + PIC_MARG_W).'px;height:'.(FEATURED_MODUL_IMAGE_HEIGHT + PIC_MARG_H).'px;">'.$p_pic.''.tep_draw_prod_feature_modul_pic_top().''.tep_draw_prod_feature_modul_pic_bottom().'</a></div>'. "\n". //
          ''. "\n".
				  '<div class="prods_padd">'.				  				  	 
				  '		<div class="name name_padd equal-height_featured_name">'.$p_name.'</div>'. "\n".
          '		<div class="desc desc_padd top">'.$p_desc.'</div>'. "\n".					
					'		<div class="price price_padd"><b>'.PRICE. '</b>'.$p_price.'</div>'. "\n".				  		
