        ...
        switch ($column_list[$col_sort]) {
          ...
          case 'PRODUCT_LIST_MANUFACTURER':
            $p_manufact = '<tr>
							...
							<td align="right"><font><a href="' . tep_href_link(FILENAME_DEFAULT, 'manufacturers_id=' . $listing['manufacturers_id']) . '">' .substr($listing['manufacturers_name'], 0, MAX_DISPLAY_MANUFACTURER_NAME_LEN).  '</a></font></td>
						  </tr>';
            break;
          case 'PRODUCT_LIST_PRICE':
            if (tep_not_null($listing['specials_new_products_price'])) {
           $p_price = $lc_text = '<span class="productSpecialPrice fl_left">' . $currencies->display_price($listing['specials_new_products_price'], tep_get_tax_rate($listing['products_tax_class_id'])) . '</span>
		   <del class="fl_right">' .  $currencies->display_price($listing['products_price'], tep_get_tax_rate($listing['products_tax_class_id'])) . '</del>';
            } else {
...
/*		$list_box_contents[$cur_row][] = array('align' => $lc_align,
			...
			if	(IMG_HOVER_EFFECT == 'style-1')	{
				$img_effect = 'first';
				$img_effect_width = PRODS_BLOCK_SEARCH_WIDTH;	
			}
    ...
    $prod_list_contents .= '<li style="width:' . PRODS_BLOCK_SEARCH_WIDTH . 'px;" class="wrapper_prods equal-height_listing_block hover">'. "\n".
				  '<div class="border_prods">'. "\n".
					...
					'<div class="view view-'.$img_effect.'">'. "\n".
					...
					'		<div class="mask" style="width:' . $img_effect_width . 'px;height:'.(SEARCH_IMAGE_HEIGHT + 7).'px;">'. "\n".
          '			  <div class="desc desc_padd top">'.$p_desc.'</div>'. "\n".
          '				<div class="button__padd bottom">'.$p_buy_now_text.'</div>'. "\n".
          '   </div>'. "\n".
          '</div>'. "\n".
...
/*		$list_box_contents[$cur_row][] = array('align' => $lc_align,
    ...
    $prod_list_contents_row .= '<li style="width:100%;" class="wrapper_prods hover">'. "\n".
				  '<div class="border_prods">'. "\n".
					...
					'<div class="view view-'.$img_effect.' fl_left" style="width:'.(SEARCH_ROW_IMAGE_WIDTH + PIC_MARG_W).'px;margin-right:14px;">'. "\n".
					...
					'		<div class="mask" style="width:' . $img_effect_width . 'px;height:'.(SEARCH_ROW_IMAGE_HEIGHT + 7).'px;">'. "\n".
          ...
          '</div>'. "\n".
				  ...
				  '			<div class="button__padd fl_left">'.$p_buy_now_text.'</div>'. "\n".
					'			<div class="price price_padd fl_left"><b>'.PRICE. '</b>'.$p_price.'</div>'. "\n".		
				  '</div>'. "\n".				  
