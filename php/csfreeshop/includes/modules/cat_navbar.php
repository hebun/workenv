...
//gt	$classname_for_parent = 'parent';
	...
	$before_html = '';   // container_'. $oscTemplate->getGridContainerWidth().'
    ...
    $after_html = '</div></div>';	
  	...
  	$title = '<div class="cat_navbar"><div class="navbar_bg">';
...
function tep_make_catsf_ullist($rootcatid = 0, $maxlevel = 0){
    ...
    // Modify category query if not fetching all categories (limit to root cats and selected subcat tree)
		...
		$result = tep_db_query('select c.categories_id, cd.categories_name, c.categories_image, c.parent_id from ' . TABLE_CATEGORIES . ' c, ' . TABLE_CATEGORIES_DESCRIPTION . ' cd where c.categories_id = cd.categories_id and cd.language_id="' . (int)$languages_id .'" '.$parent_query.' order by sort_order, cd.categories_name');
    
		while ($row = tep_db_fetch_array($result)) {				
        $table[$row['parent_id']][$row['categories_id']]['parent'] = $row['parent_id'];
        $table[$row['parent_id']][$row['categories_id']]['name'] = $row['categories_name'];
        $table[$row['parent_id']][$row['categories_id']]['image'] = $row['categories_image'];
    }
...
$b = 'item-';
    ...
    while(list($key,$val) = each($list)){
        ...
        if ($GLOBALS['this_level'] != $level) {
		        ...
		        if ($GLOBALS['this_level'] < $level) {
						 switch (SUPERFISH_COLUMN) {
							case 0:
									$ul_width	= ' style="width:16em;"';
									break;
							case 1:
									$ul_width	= ' style="width:16em;"';
									break;
							case 2:
									$ul_width	= ' style="width:34em;"';
									break;
							case 3:
									$ul_width	= ' style="width:50em;"';
									break;
						}
							
					 if (($level) ==1) {
						 		$elements_count = count($table[$val['parent']]);
				        $output .= "\n".'<div class="div_cat_navbar"><div class="marker"></div><ul '.$ul_width.'>';
					 }else{
						$output .= "\n".'<ul '.$ul_width.'>'; 
					 }
				} else {
	...
	*/							
     ...
     //gt   $output .= '<li class="cat_lev_'.$level.'"><a href="';
		...
		if ($GLOBALS['this_level'] == 1) {
				if ($caunt == SUPERFISH_COLUMN ) {//SUPERFISH_COLUMN
				$output .= "\n".'</ul><ul '.$ul_width.'>';
				$caunt_ul ++;
				$caunt = 0;
				}
				...
				/*
				if ($elements_count % SUPERFISH_COLUMN == 0 )	{
					 $li_count = floor($elements_count/SUPERFISH_COLUMN);
				}else{
					 $li_count = floor($elements_count/SUPERFISH_COLUMN) + 1;
				}
				...
				*/
				//$caunt_ul = 1;
				 ...
				 if ($level == 0)	{
					$wrapper_level1 ='<div class="wrapper_level">';
					$wrapper_level2 ='</div>';
					
				 }
         $output .= '<li class="current_'.$level.' '.$b.$a.'"><a href="';
				 $a ++;
        ...
        if (tep_has_category_subcategories($key) && $classname_for_parent) {
            ...
            $this_parent_class = '';		
				...
				$categories_name = ((strlen($val['name']) > MAX_DESCR_CATEGORY_NAME) ? substr($val['name'], 0, MAX_DESCR_CATEGORY_NAME) . '...' : $val['name']);
        $output .= tep_href_link(FILENAME_DEFAULT, $cPath_new) . '"'.$this_parent_class.' rel="'.DIR_WS_IMAGES . $val['image'].'">'.$wrapper_level1.''.$categories_name.'';		
